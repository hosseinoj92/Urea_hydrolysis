...
CONFIG = {
    ...
    "output_dir": r"C:\Users\vt4ho\Simulations\simulation_data\generated_data\imperfect\version_experiment\Generated_Data_EarlyInference_50000",  # Output directory
    "time_model": "uniform",

    # Prefix extraction (multiple prefix lengths for training)
    "prefix_lengths": [30.0, 300.0],  # Prefix lengths in seconds [s]
    ...
    # Parameters to infer (powder_activity_frac and k_d)
    "infer_params": [
        "powder_activity_frac",  # Fraction of powder that is active enzyme [0-1]
    ...
    "param_ranges": {
        ...
        "substrate_mM": 10.0,  # Uniform range: [min, max]
        "grams_urease_powder": [0.0094,0.011],  # Uniform range: [min, max]
        "temperature_C": [25.0, 30.0, 35.0, 40.0],  # Uniform range: [min, max]
        ...
        "initial_pH": [6.5, 7.2],  # Uniform range: [min, max]
        # volume_L is fixed (see fixed_params below) - NOT sampled
        
        # Latent parameters to infer (powder_activity_frac and k_d)
        "powder_activity_frac": [0.01, 1.0],  # Fraction of powder that is active enzyme [0-1] (1% to 100%)
...
def _worker_generate_single_trajectory(args_tuple):
    ...
    Parameterization: samples powder_activity_frac, computes E0_g_per_L from it.
    ...
    try:
        ...
        # Extract latent parameters to infer (powder_activity_frac and k_d)
        powder_activity_frac = params_dict["powder_activity_frac"]
        k_d = params_dict["k_d"]
        
        # Compute effective enzyme loading from powder activity fraction
        # E0_g_per_L = (active enzyme mass) / volume = (powder_activity_frac * grams_urease_powder) / volume_L
        E0_g_per_L = powder_activity_frac * grams_urease_powder / volume_L
        ...
        # Target parameters (to infer - powder_activity_frac and k_d)
...
def main():
    ...
    for prefix_length in CONFIG["prefix_lengths"]:
        ...
        processed_data[prefix_length] = {
            ...
            "target_params": np.array(target_params_array),  # (n_examples, 2) - powder_activity_frac, k_d
        }
    ...
    metadata = {
        ...
        "infer_params": CONFIG["infer_params"],  # ["powder_activity_frac", "k_d"]
